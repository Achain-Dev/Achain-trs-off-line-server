...
import static com.achain.data.Operation.OperationType.CALL_CONTRACT_OP_TYPE;
import static com.achain.data.Operation.OperationType.DEPOSIT_OP_TYPE;
import static com.achain.data.Operation.OperationType.IMESSAGE_MEMO_OP_TYPE;
import static com.achain.data.Operation.OperationType.TRANSFER_CONTRACT_OP_TYPE;
import static com.achain.data.Operation.OperationType.WITHDRAW_OP_TYPE;
...
public class Operation {
    ...
    operation.setJson(
        JSON.build()
            ...
            .add("data", JSON.build()
                             ...
                             .add("amount", amount)
                             .add("balance_id", new ACTAddress(balanceId, ACTAddress.Type.BALANCE_ID).getAddressStrStartWithSymbol())
                             .get())
            .get());
    return operation;
  }
  ...
  public static Operation createCallContract(
    ...
    operation.setJson(
        JSON.build()
            ...
            .add("data", JSON.build()
                             .add("caller", new ACTAddress(actPrivateKey.getPublicKey(true), ACTAddress.Type.PUBLIC_KEY)
  ...
  public static Operation createCallContract(
      ...
      String args,
      Asset costLimit) {
    ACTAddress actAddress = new ACTAddress(contractId, ACTAddress.Type.CONTRACT);
    Map<byte[], Long> balances = new HashMap<>();
    Asset transactionFee = new Asset(Transaction.requiredFees);
    balances.put(new WithdrawCondition(actPrivateKey.getAddress()).getBalanceId(),
                 costLimit.getAmount() + transactionFee.getAmount());
  ...
  public static Operation createContractTransaction(ACTPrivateKey actPrivateKey, String contractId, Asset costLimit, Asset amount){
    ...
    operation.setJson(
            JSON.build()
                    ...
                    .add("data", JSON.build()
                            .add("from", new ACTAddress(actPrivateKey.getPublicKey(true), ACTAddress.Type.PUBLIC_KEY)
                                    .getAddressStrStartWithSymbol())
                            .add("costlimit", costLimit.toJSON())
                            .add("transaction_fee", transactionFee.toJSON())
  ...
  private static List<String[]> mapFlatToList(Map<byte[], Long> balances) {
    ...
    balances.forEach((k, v) -> result.add(
        new String[]{
            new ACTAddress(k, ACTAddress.Type.BALANCE_ID).getAddressStrStartWithSymbol(),
            v.toString()
        }));
    return result;
  }
